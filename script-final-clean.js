...
async function initializeDashboard() {
    try {
        ...
        document.getElementById("generate-button")?.addEventListener("click", async () => {
            const data = await generateMultipleGroups();  // ✅ ใช้ generator ที่เราทำไว้
            const container = document.getElementById("suggestions-section");
            container.innerHTML = '';

            const strategyMap = {
                'best-heatmap': 'BEST HEATMAP',
                'cold-random': 'COLD RANDOM',
                'pure-random': 'PURE RANDOM',
                'heat-bias': 'HEAT BIAS',
                'bias-matrix': 'BIAS MATRIX',
                'pattern-biased': 'PATTERN BIASED'
            };

            Object.entries(strategyMap).forEach(([key, label]) => {
                ...
                (data[key] || []).forEach(result => {
                    ...
                    const digits = result.number.split('').reverse();
                    ...
                    desc.innerHTML = `🔥 ${result.description}`;  // ✅ ใช้ description ที่ generate แบบเต็ม
...
async function loadSuggestions() {
    ...
    for (const [mode, suggestions] of Object.entries(data)) {
        ...
        suggestions.forEach(sugg => {
            ...
            ? `🔥 ${sugg.description}` 
            : `🔥 ${Math.round(sugg.confidence * 100)}% - Generated by heatmap`;
